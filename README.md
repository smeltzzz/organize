...
[![Tests](https://img.shields.io/badge/tests-1346%20passing%20(offline)-2EA44F.svg?style=flat-square)](.github/workflows/ci.yml)
...
| `tests/` | Fully offline unit tests (1,346), including `tests/selftests/` — each tool's own suite, moved out of the shipped file — plus `fake_mkvmerge.py` and `fake_ffprobe.py`, stand-in binaries real enough to drive an end-to-end remux and an end-to-end library inspection. |
