...
[![Tests](https://img.shields.io/badge/tests-1552%20passing%20(offline)-2EA44F.svg?style=flat-square)](.github/workflows/ci.yml)
...
| `tests/` | Fully offline unit tests (1,552), including `tests/selftests/` — each tool's own suite, moved out of the shipped file — plus `fake_mkvmerge.py`, `fake_ffprobe.py` and `fake_provider.py`, stand-ins real enough to drive an end-to-end remux, library inspection and subtitle fetch. |
