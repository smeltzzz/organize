...
* `movie_standardizer` naming — the output of one tool is the input to the
  ...
  name would show up.
* `subtitle_fetcher` — the only tool that acts on input from strangers. Its
  planners decide which of a provider's answers gets written next to a movie,
  and its quota arithmetic is a promise to that provider; both are pure
  functions of remote data, which is exactly the shape a table of examples
  covers worst.
...
# ---------------------------------------------------------------------------
# The subtitle fetcher: choosing an answer from a stranger, and paying for it
# ---------------------------------------------------------------------------

TITLES = ("The Dark Knight", "Heat", "Alien", "Up", "Se7en", "Solaris")
SOURCE_TAGS = ("BluRay", "Blu-ray", "BDRip", "1080p", "qXR", "Tigole", "WEBRip",
               "HDTV", "DVDRip", "")
EDITIONS = ("", "", "", "Directors.Cut", "Extended", "Theatrical.Cut", "REMASTERED")
LANGUAGES = ("en", "eng", "english", "EN", "es", "fr", "pt-BR", "", "de")

IDENTITY = sf.MovieIdentity("The Dark Knight", 2008,
                            sf.normalize_title("The Dark Knight"))


def provider_release(rng: Any) -> str:
    """A release string of the shape providers actually return.

    Deliberately weighted towards the movie being asked about: a strategy that
    almost never produces an acceptable candidate would make every property
    below pass by refusing everything, which proves nothing at all.
    """
    title = (IDENTITY.title if rng.random() < 0.7 else rng.choice(TITLES)).replace(" ", ".")
    year = IDENTITY.year if rng.random() < 0.7 else rng.choice((1995, 1979, 0))
    parts = [title]
    if year:
    ...
    tag = (rng.choice(("BluRay", "Blu-ray", "1080p", "qXR", "Tigole", "BDRip"))
           if rng.random() < 0.7 else rng.choice(SOURCE_TAGS))
    if tag:
        parts.append(tag)
    parts.append(rng.choice(("x264-GROUP", "x265-qXR", "AAC", "")))
    return ".".join(part for part in parts if part)


def candidate(rng: Any) -> sf.Candidate:
    return sf.Candidate(
        file_id=rng.randint(1, 40),
        release=provider_release(rng),
        moviehash_match=rng.random() < 0.7,
        downloads=rng.randint(0, 5000),
        votes=rng.randint(0, 200),
        rating=round(rng.uniform(0, 10), 1),
        trusted=rng.random() < 0.4,
        hearing_impaired=rng.random() < 0.3,
        machine_translated=rng.random() < 0.12,
        ai_translated=rng.random() < 0.12,
        foreign_parts_only=rng.random() < 0.12,
        language=(rng.choice(("en", "eng", "english", "EN")) if rng.random() < 0.75
                  else rng.choice(LANGUAGES)),
        feature_title=IDENTITY.title if rng.random() < 0.7 else rng.choice(TITLES),
        feature_year=IDENTITY.year if rng.random() < 0.7 else rng.choice((1995, 1979, 0)),
        subdl_match_score=rng.choice((None, 0.0, 0.5, 0.79, 0.8, 0.9, 0.95, 1.0, 2.0, -1.0)),
    )


def refused_candidate(rng: Any) -> sf.Candidate:
    """One the policy must never install, whatever else is in the list."""
    poisoned = candidate(rng)
    return replace(
        poisoned,
        language=rng.choice(("es", "fr", "de")),
        machine_translated=True,
        ai_translated=rng.random() < 0.5,
        foreign_parts_only=rng.random() < 0.5,
    )


def candidates(rng: Any) -> list[sf.Candidate]:
    """A provider's answer list, which really does contain near-duplicates.

    Two uploads of the same release differing only in id is the ordinary case
    on OpenSubtitles, and it is the case the tie-breaking rules exist for: pure
    random candidates almost never collide on a rank key, so a generator
    without clones would leave "a tie is a review" and "the order does not
    matter" both passing vacuously.
    """
    found = [candidate(rng) for _ in range(rng.randint(0, 5))]
    if found and rng.random() < 0.5:
        twin = replace(rng.choice(found), file_id=rng.randint(41, 80))
        found.insert(rng.randint(0, len(found)), twin)
    return found


def scrape_candidate(rng: Any) -> sf.ScrapeCandidate:
    return sf.ScrapeCandidate(
        provider=rng.choice(sf.SCRAPE_PROVIDER_ORDER),
        file_id=str(rng.randint(1, 50)),
        release=provider_release(rng),
        feature_title=(IDENTITY.title if rng.random() < 0.6
                       else rng.choice(TITLES + ("",))),
        feature_year=IDENTITY.year if rng.random() < 0.6 else rng.choice((1995, 0)),
        downloads=rng.randint(0, 999),
        rating=round(rng.uniform(0, 10), 1),
    )


class FetcherSelectionTests(PropertyTestCase):
    """What may be written next to a movie, for any answer a provider gives."""

    def test_the_chosen_subtitle_is_always_one_that_was_offered(self) -> None:
        def prop(cands: list[sf.Candidate]) -> None:
            pick = sf.pick_candidate(cands, sf.Config(), identity=IDENTITY)
            if pick is not None:
                self.assertIn(pick, cands, "the pick was not in the provider's list")
        self.for_all(candidates, prop)

    def test_a_translated_or_non_english_subtitle_is_never_chosen(self) -> None:
        """Machine translation is the one thing worse than no subtitle."""
        def prop(cands: list[sf.Candidate]) -> None:
            for pick in (sf.pick_candidate(cands, sf.Config(), identity=IDENTITY),
                         sf.pick_identity_candidate(cands, IDENTITY)[0],
                         sf.pick_subdl_identity_candidate(cands, IDENTITY)[0]):
                if pick is None:
                    continue
                self.assertIn(sf.normalize_language(pick.language),
                              sf.ENGLISH_LANGUAGE_TOKENS)
                self.assertFalse(pick.machine_translated)
                self.assertFalse(pick.ai_translated)
                self.assertFalse(pick.foreign_parts_only)
        self.for_all(candidates, prop)

    def test_the_hash_route_only_ever_installs_a_hash_match(self) -> None:
        """A moviehash match is proof of *this* file; nothing else is."""
        def prop(cands: list[sf.Candidate]) -> None:
            pick = sf.pick_candidate(cands, sf.Config(), identity=IDENTITY)
            if pick is not None:
                self.assertTrue(pick.moviehash_match)
                self.assertTrue(sf.release_matches_movie_identity(pick.release, IDENTITY))
                self.assertTrue(sf.release_has_library_source_keyword(pick.release))
        self.for_all(candidates, prop)

    def test_the_order_the_provider_listed_them_does_not_change_the_pick(self) -> None:
        def prop(cands: list[sf.Candidate]) -> None:
            first = sf.pick_candidate(cands, sf.Config(), identity=IDENTITY)
            second = sf.pick_candidate(list(reversed(cands)), sf.Config(), identity=IDENTITY)
            self.assertEqual(first, second)
        self.for_all(candidates, prop)

    def test_offering_a_refused_subtitle_as_well_changes_nothing(self) -> None:
        """A candidate the policy rejects cannot displace one it accepts."""
        def prop(pair: tuple[list[sf.Candidate], sf.Candidate]) -> None:
            cands, poison = pair
            before = sf.pick_candidate(cands, sf.Config(), identity=IDENTITY)
            after = sf.pick_candidate([poison, *cands], sf.Config(), identity=IDENTITY)
            self.assertEqual(before, after)
        self.for_all(lambda rng: (candidates(rng), refused_candidate(rng)), prop)

    def test_an_identity_pick_matches_the_provider_metadata_exactly(self) -> None:
        """No hash to lean on, so title, year and release name must all agree."""
        def prop(cands: list[sf.Candidate]) -> None:
            pick, reason = sf.pick_identity_candidate(cands, IDENTITY)
            if pick is None:
                self.assertTrue(reason, "a refusal always says why")
                return
            self.assertEqual(pick.feature_year, IDENTITY.year)
            self.assertEqual(sf.normalize_title(pick.feature_title),
                             IDENTITY.normalized_title)
            self.assertFalse(sf.release_has_edition_marker(pick.release),
                             "a cut this library cannot verify is never auto-selected")
        self.for_all(candidates, prop)

    def test_an_unbroken_tie_is_held_for_review_rather_than_guessed(self) -> None:
        def prop(cands: list[sf.Candidate]) -> None:
            pick, _reason = sf.pick_identity_candidate(cands, IDENTITY)
            if pick is None:
                return
            top = sf.candidate_rank_key(pick)
            tied = [c for c in cands if sf.candidate_rank_key(c) == top
                    and sf.pick_identity_candidate([c], IDENTITY)[0] is not None]
            self.assertEqual(len(tied), 1, "two equally ranked answers is a review")
        self.for_all(candidates, prop)

    def test_a_subdl_release_match_is_never_below_the_documented_threshold(self) -> None:
        """SubDL's score is the whole claim, so the floor is not negotiable."""
        def prop(cands: list[sf.Candidate]) -> None:
            pick, _reason = sf.pick_subdl_identity_candidate(
                cands, IDENTITY, require_release_match_score=True)
            if pick is None:
                return
            score = sf._subdl_match_score(pick.subdl_match_score)
            self.assertIsNotNone(score)
            self.assertGreaterEqual(score, sf.MIN_SUBDL_RELEASE_MATCH_SCORE)
        self.for_all(candidates, prop)

    def test_pooling_two_providers_never_invents_a_third_answer(self) -> None:
        def prop(pair: tuple[sf.Candidate, sf.Candidate]) -> None:
            entries = [(pair[0], sf.PROVIDER_OPENSUBTITLES, "hash", "os"),
                       (pair[1], sf.PROVIDER_SUBDL, "release", "subdl")]
            pick, provider, _method, reason = sf.pick_pooled_candidates(entries, IDENTITY)
            if pick is None:
                self.assertTrue(reason)
                return
            self.assertIn((pick, provider), [(c, p) for c, p, _m, _r in entries])
        self.for_all(lambda rng: (candidate(rng), candidate(rng)), prop)

    def test_the_pooled_winner_is_never_the_worse_ranked_of_the_two(self) -> None:
        """Equal sources: the release wins on its merits, not on its provider."""
        def prop(pair: tuple[sf.Candidate, sf.Candidate]) -> None:
            entries = [(pair[0], sf.PROVIDER_OPENSUBTITLES, "hash", "os"),
                       (pair[1], sf.PROVIDER_SUBDL, "release", "subdl")]
            pick, _provider, _method, _reason = sf.pick_pooled_candidates(entries, IDENTITY)
            if pick is None:
                return
            others = [c for c, _p, _m, _r in entries
                      if c is not pick
                      and sf.release_has_library_source_keyword(c.release)
                      and sf.release_matches_movie_identity(c.release, IDENTITY)]
            for other in others:
                self.assertLessEqual(sf.candidate_rank_key(pick),
                                     sf.candidate_rank_key(other))
        self.for_all(lambda rng: (candidate(rng), candidate(rng)), prop)

    def test_a_scraped_shortlist_is_a_subset_that_names_the_movie(self) -> None:
        def prop(cands: list[sf.ScrapeCandidate]) -> None:
            identity = sf.SourceIdentity(IDENTITY.title, IDENTITY.year,
                                         IDENTITY.normalized_title)
            picked = sf.pick_candidates(identity, cands)
            self.assertLessEqual(len(picked), sf.SCRAPE_MAX_CANDIDATES_PER_SOURCE)
            for cand in picked:
                self.assertIn(cand, cands)
                if cand.feature_year:
                    self.assertEqual(cand.feature_year, identity.year)
        self.for_all(lists(scrape_candidate, max_size=8), prop)


class DownloadUrlTests(PropertyTestCase):
    """The one place a remote string could become a request to anywhere."""

    def test_a_download_url_is_dl_subdl_com_over_https_or_it_is_refused(self) -> None:
        def prop(raw: str) -> None:
            try:
                url = sf.normalize_subdl_download_url(raw)
            except ValueError:
                return
            # Spelled out rather than read back from the module: an oracle
            # that asks the code under test what it should have done cannot
            # fail. See MutationTests below, which relies on exactly that.
            self.assertTrue(url.startswith("https://dl.subdl.com/subtitle/"),
                            f"{raw!r} was normalised to {url!r}")
        self.for_all(one_of(
            text("abcdefghij:/.@?#%0123456789-", max_size=24),
            lambda rng: rng.choice((
                "/subtitle/1.zip", "//evil.example/subtitle/1.zip",
                "https://dl.subdl.com/subtitle/1.zip",
                "https://dl.subdl.com:8080/subtitle/1.zip",
                "https://user:pw@dl.subdl.com/subtitle/1.zip",
                "http://dl.subdl.com/subtitle/1.zip",
                "https://dl.subdl.com.evil.example/subtitle/1.zip",
                "https://dl.subdl.com/subtitle/../../etc/passwd",
                "/subtitle/%2e%2e/%2e%2e/etc/passwd", "file:///etc/passwd", "",
            )),
        ), prop, cases=200)


class QuotaArithmeticTests(PropertyTestCase):
    """A daily cap is a promise to a provider, not a suggestion."""

    def config(self, **kw: Any) -> sf.QueueConfig:
        return sf.QueueConfig(library=Path("/library"), log_file=None,
                              report_file=Path("/report.txt"), **kw)

    def test_a_cap_is_never_exceeded_however_many_movies_ask(self) -> None:
        def prop(plan: tuple[int, int]) -> None:
            cap, movies = plan
            cfg = self.config(daily_cap=cap, subdl_daily_cap=cap)
            ledger: dict[str, int] = {}
            spent = 0
            for _ in range(movies):
                for provider in (sf.PROVIDER_OPENSUBTITLES, sf.PROVIDER_SUBDL):
                    if sf.provider_has_quota(cfg, ledger, provider):
                        sf.reserve_provider_download(ledger, provider)
                        spent += 1
            for provider in (sf.PROVIDER_OPENSUBTITLES, sf.PROVIDER_SUBDL):
                self.assertLessEqual(sf.provider_reserved(ledger, provider), max(cap, 0))
            self.assertLessEqual(spent, 2 * max(cap, 0))
        self.for_all(lambda rng: (rng.randint(0, 6), rng.randint(0, 8)), prop)

    def test_a_reservation_is_recorded_before_it_can_be_used(self) -> None:
        """Interrupt the run at any point and the ledger has already paid."""
        def prop(steps: int) -> None:
            ledger: dict[str, int] = {}
            for expected in range(1, steps + 1):
                self.assertEqual(
                    sf.reserve_provider_download(ledger, sf.PROVIDER_OPENSUBTITLES),
                    expected)
                self.assertEqual(sf.provider_reserved(ledger, sf.PROVIDER_OPENSUBTITLES),
                                 expected)
        self.for_all(integers(0, 12), prop)

    def test_the_two_subdl_allowances_are_metered_apart(self) -> None:
        def prop(counts: tuple[int, int]) -> None:
            downloads, searches = counts
            ledger: dict[str, int] = {}
            for _ in range(downloads):
                sf.reserve_provider_download(ledger, sf.PROVIDER_SUBDL)
            for _ in range(searches):
                sf.reserve_subdl_search(ledger)
            self.assertEqual(sf.provider_reserved(ledger, sf.PROVIDER_SUBDL), downloads)
            self.assertEqual(sf.subdl_search_reserved(ledger), searches)
            self.assertEqual(sf.provider_reserved(ledger, sf.PROVIDER_OPENSUBTITLES), 0)
        self.for_all(lambda rng: (rng.randint(0, 5), rng.randint(0, 5)), prop)

    def test_an_exhausted_wallet_offers_no_source_at_all(self) -> None:
        def prop(plan: dict[str, Any]) -> None:
            cfg = self.config(
                daily_cap=plan["open_cap"], subdl_daily_cap=plan["subdl_cap"],
                subdl_search_daily_cap=plan["search_cap"],
                scrape_daily_cap=plan["scrape_cap"])
            ledger = {"opensubtitles_download_requests_reserved": plan["open_used"],
                      "subdl_download_requests_reserved": plan["subdl_used"],
                      "subdl_search_requests_reserved": plan["search_used"]}
            sources = sf.plan_sources(
                cfg, ledger, sf.HistoryPlan(),
                has_open=plan["has_open"], has_subdl=plan["has_subdl"],
                has_scrape_chain=plan["scrape_cap"] > 0,
                scrape_keys=sf.SCRAPE_PROVIDER_ORDER if plan["scrape_cap"] > 0 else ())
            if sources.open_available:
                self.assertTrue(plan["has_open"])
                self.assertLess(plan["open_used"], plan["open_cap"])
            if sources.subdl_available:
                self.assertTrue(plan["has_subdl"])
                self.assertLess(plan["subdl_used"], plan["subdl_cap"])
                self.assertLess(plan["search_used"], plan["search_cap"])
            self.assertEqual(
                sources.exhausted,
                not (sources.open_available or sources.subdl_available
                     or sources.scrape_available))
        self.for_all(fixed_dict({
            "open_cap": integers(0, 3), "subdl_cap": integers(0, 3),
            "search_cap": integers(0, 3), "scrape_cap": integers(0, 2),
            "open_used": integers(0, 3), "subdl_used": integers(0, 3),
            "search_used": integers(0, 3),
            "has_open": booleans(), "has_subdl": booleans(),
        }), prop)


class HistoryPlanTests(PropertyTestCase):
    """What a movie's own record is allowed to cost before anything is asked."""

    def record(self, rng: Any) -> dict[str, Any]:
        day = rng.choice(("2026-01-01", "2026-01-02"))
        return {
            "status": rng.choice(("pending", "no_match", "manual_review", "reserved",
                                  "covered", "")),
            "updated_utc": f"{day}T04:05:06+00:00",
            "scrape_failed": rng.random() < 0.5,
            "scrape_failed_utc_day": rng.choice(("", "2026-01-01", "2026-01-02")),
            "scrape_checked": rng.random() < 0.5,
            "providers_checked": rng.choice(
                ([], ["opensubtitles"], ["opensubtitles", "subdl"], None)),
        }

    def plan(self, record: dict[str, Any], **kw: Any) -> sf.HistoryPlan:
        options = {"today": "2026-01-02", "retry_no_match": False,
                   "identity_fallback": True,
                   "scrape_keys": sf.SCRAPE_PROVIDER_ORDER,
                   "active_providers": ("opensubtitles", "subdl")}
        options.update(kw)
        return sf.plan_from_history(record, **options)

    def test_the_plan_is_always_one_of_the_three_things_it_may_say(self) -> None:
        def prop(record: dict[str, Any]) -> None:
            plan = self.plan(record)
            self.assertIn(plan.action, ("fetch", "skip", "review"))
            if not plan.fetch:
                self.assertTrue(plan.detail and plan.reason,
                                "a movie that costs nothing still says why")
        self.for_all(self.record, prop)

    def test_a_download_reserved_today_is_never_reserved_again_today(self) -> None:
        """The ledger already paid for it; the next UTC day gets the retry."""
        def prop(record: dict[str, Any]) -> None:
            record = {**record, "status": "reserved",
                      "updated_utc": "2026-01-02T01:00:00+00:00",
                      "scrape_failed": False}
            self.assertFalse(self.plan(record).fetch)
        self.for_all(self.record, prop)

    def test_the_scraping_tier_is_offered_at_most_once_a_day(self) -> None:
        def prop(record: dict[str, Any]) -> None:
            plan = self.plan(record)
            if plan.scrape_tried_today:
                self.assertFalse(plan.scrape_retry_today,
                                 "already exhausted today, so not retried today")
        self.for_all(self.record, prop)

    def test_a_review_hold_is_honoured_until_something_actually_changed(self) -> None:
        """Otherwise every run re-buys the same subtitle it already refused."""
        def prop(record: dict[str, Any]) -> None:
            record = {**record, "status": "manual_review",
                      "providers_checked": ["opensubtitles", "subdl"],
                      "scrape_checked": True,
                      "scrape_failed": True,
                      "scrape_failed_utc_day": "2026-01-02"}
            plan = self.plan(record, retry_no_match=False)
            self.assertFalse(plan.fetch)
        self.for_all(self.record, prop)
...
class MutationTests(unittest.TestCase):
    ...
    def test_it_notices_when_a_translated_subtitle_can_be_installed(self) -> None:
        self.assert_property_notices(
            FetcherSelectionTests, "test_a_translated_or_non_english_subtitle_is_never_chosen",
            (sf, "_is_normal_english_human_candidate", lambda candidate: True),
        )

    def test_it_notices_when_a_download_url_may_leave_the_provider(self) -> None:
        self.assert_property_notices(
            DownloadUrlTests, "test_a_download_url_is_dl_subdl_com_over_https_or_it_is_refused",
            (sf, "SUBDL_DOWNLOAD_HOST", "evil.example"),
        )

    def test_it_notices_when_a_daily_cap_stops_being_enforced(self) -> None:
        self.assert_property_notices(
            QuotaArithmeticTests, "test_a_cap_is_never_exceeded_however_many_movies_ask",
            (sf, "provider_has_quota", lambda cfg, ledger, provider: True),
        )

    def test_it_notices_when_a_reserved_download_is_bought_twice(self) -> None:
        self.assert_property_notices(
            HistoryPlanTests, "test_a_download_reserved_today_is_never_reserved_again_today",
            (sf, "plan_from_history", lambda record, **kw: sf.HistoryPlan()),
        )
