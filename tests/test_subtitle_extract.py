...
class ConversionTests(unittest.TestCase):
    ...
    def test_usf_xml_becomes_srt(self) -> None:
        ...
        converted = sf.usf_to_srt(USF_TRACK)
        self.assertIn("00:00:01,000 --> 00:00:03,000", converted)
    ...
    def test_a_usf_cue_with_an_unreadable_time_is_skipped(self) -> None:
        self.assertNotIn("no timing", sf.usf_to_srt(USF_TRACK))

    def test_usf_that_is_not_usf_yields_no_cues(self) -> None:
        self.assertEqual(sf.usf_to_srt("<html><body>nope</body></html>"), "")
...
class ChoosingAnOcrBackendTests(unittest.TestCase):
    ...
    image-only movie just falls through to the providers — so what matters as
    much as the choice is that the note explains the fix.
    """
    ...
    def nothing_on_path(self) -> None:
        patcher = mock.patch.object(sf.shutil, "which", lambda _name: None)
        patcher.start()
        self.addCleanup(patcher.stop)

    def on_path(self, **programs: str) -> None:
        patcher = mock.patch.object(sf.shutil, "which", lambda name: programs.get(name))
        patcher.start()
        self.addCleanup(patcher.stop)
    ...
    def test_a_path_handed_in_is_used_as_it_is(self) -> None:
        ...
        backend = sf.build_ocr_backend(sf.OCR_BACKEND_SUP2SRT, str(program))
    ...
    def test_a_name_handed_in_is_looked_up_on_the_path(self) -> None:
        ...
        backend = sf.build_ocr_backend(sf.OCR_BACKEND_SUP2SRT, "my-sup2srt")
        self.assertIsNotNone(backend)
        assert backend is not None
    ...
    def test_a_known_install_location_is_tried_after_the_path(self) -> None:
        ...
        self.assertEqual(sf._resolve_program("", "SubtitleEdit", str(installed)),
                         str(installed))
    ...
    def test_a_windows_subtitle_edit_is_run_through_mono(self) -> None:
        ...
        backend = sf.build_ocr_backend(sf.OCR_BACKEND_SUBTITLEEDIT, str(exe))
        self.assertIsNotNone(backend)
        assert backend is not None
    ...
    def test_subtitle_edit_without_mono_counts_as_not_installed(self) -> None:
        self.nothing_on_path()
        exe = self.program("SubtitleEdit.exe")
        self.assertIsNone(sf.build_ocr_backend(sf.OCR_BACKEND_SUBTITLEEDIT, str(exe)))
    ...
    def test_a_native_subtitle_edit_is_run_directly(self) -> None:
        ...
        backend = sf.build_ocr_backend(sf.OCR_BACKEND_SUBTITLEEDIT)
        assert backend is not None
    ...
    def test_pgsrip_and_pgstosrt_are_built_when_present(self) -> None:
        ...
        pgsrip = sf.build_ocr_backend(sf.OCR_BACKEND_PGSRIP)
        ...
        pgstosrt = sf.build_ocr_backend(sf.OCR_BACKEND_PGSTOSRT, str(dll))
    ...
    def test_a_backend_this_tool_does_not_know_is_not_built(self) -> None:
        self.assertIsNone(sf.build_ocr_backend("hand-typed"))
    ...
    def test_auto_takes_the_first_backend_it_finds(self) -> None:
        ...
        backend, note = sf.detect_ocr_backend(sf.OCR_BACKEND_AUTO)
        self.assertIsNotNone(backend)
        assert backend is not None
        self.assertEqual(backend.key, sf.OCR_BACKEND_SUP2SRT)
    ...
    def test_a_named_backend_is_the_only_one_tried(self) -> None:
        self.on_path(sup2srt="/usr/bin/sup2srt")
        backend, note = sf.detect_ocr_backend(sf.OCR_BACKEND_PGSRIP)
    ...
    def test_a_named_backend_that_is_installed_is_used(self) -> None:
        self.on_path(sup2srt="/usr/bin/sup2srt")
        backend, note = sf.detect_ocr_backend(sf.OCR_BACKEND_SUP2SRT)
        self.assertIsNotNone(backend)
        self.assertEqual(note, "")

    def test_a_backend_name_that_does_not_exist_is_refused(self) -> None:
        backend, note = sf.detect_ocr_backend("tesseract-by-hand")
        self.assertIsNone(backend)
    ...
    def test_a_custom_command_needs_both_placeholders(self) -> None:
        ...
        for template in ("--in {input}", "--out {output}", "--quiet"):
            with self.subTest(template=template):
                backend, note = sf.detect_ocr_backend(
                    sf.OCR_BACKEND_CUSTOM, explicit_bin=str(program),
...
class RunningTheOcrTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.backend = sf.OcrBackend(sf.OCR_BACKEND_SUP2SRT, "sup2srt + Tesseract",
                                     ("sup2srt",), frozenset({"PGS"}))
        self.sibling_backend = sf.OcrBackend(
            sf.OCR_BACKEND_PGSRIP, "pgsrip + Tesseract", ("pgsrip",),
            frozenset({"PGS"}), output_mode="sibling")
    ...
    def command_that(self, rc: int = 0, writes: Path | None = None,
        ...
        return mock.patch.object(sf, "run_external_command", side_effect=fake)

    def test_output_where_it_was_asked_for_is_a_success(self) -> None:
        with self.command_that(writes=self.output):
            ok, detail = sf.run_ocr(self.backend, self.source, self.output)
        self.assertTrue(ok)
    ...
    def test_output_written_beside_the_input_is_collected(self) -> None:
        ...
        with self.command_that(writes=beside):
            ok, detail = sf.run_ocr(self.sibling_backend, self.source, self.output)
    ...
    def test_output_that_cannot_be_collected_is_reported(self) -> None:
        beside = self.tmp / "track3.eng.srt"
        with self.command_that(writes=beside), \
                mock.patch.object(sf.shutil, "move", side_effect=OSError("read-only")):
            ok, detail = sf.run_ocr(self.sibling_backend, self.source, self.output)
        self.assertFalse(ok)
    ...
    def test_an_empty_result_is_not_a_subtitle(self) -> None:
        with self.command_that(writes=self.output, text=""):
            ok, _detail = sf.run_ocr(self.backend, self.source, self.output)
        self.assertFalse(ok)
...
class OneTrackAtATimeTests(unittest.TestCase):
    ...
    Extraction is the cheapest way to cover a movie — no provider, no quota,
    no network — but only if what comes out of the container is really the
    ...
    def run_with(self, runner: object, *, backend: object = None,
                 **options: object) -> sf.ExtractionOutcome:
        opts = sf.ExtractOptions(min_cues=2, **options)  # type: ignore[arg-type]
        patches = [mock.patch.object(subprocess, "run", runner),
                   mock.patch.object(sf, "find_mkvtoolnix_binary", fake_binaries)]
        if backend is not None:
            patches.append(mock.patch.object(sf, "detect_ocr_backend",
    ...
    def test_a_sidecar_that_cannot_be_written_is_reported(self) -> None:
        with mock.patch.object(sf, "atomic_write_text",
    ...
    def ocr_backend(self) -> sf.OcrBackend:
        return sf.OcrBackend(sf.OCR_BACKEND_SUP2SRT, "sup2srt + Tesseract", ("sup2srt",),
                             frozenset({"PGS"}))

    def test_an_ocred_image_track_becomes_a_sidecar(self) -> None:
        text = sf.render_srt_cues([
        ...
        with mock.patch.object(sf, "run_ocr", side_effect=ocr):
    ...
    def test_an_ocr_failure_is_carried_into_the_reason(self) -> None:
        with mock.patch.object(sf, "run_ocr",
    ...
    def test_an_ocr_result_that_cannot_be_read_is_a_failure(self) -> None:
        ...
        with mock.patch.object(sf, "run_ocr", return_value=(True, "")):
            outcome = self.run_with(FakeRunner(PGS_TRACKS), backend=self.ocr_backend())
        self.assertFalse(outcome.ok)
    ...
    def test_a_backend_that_cannot_read_the_codec_is_not_run(self) -> None:
        vobsub_only = sf.OcrBackend(sf.OCR_BACKEND_SUP2SRT, "sup2srt", ("sup2srt",),
                                    frozenset({"VOBSUB"}))
        with mock.patch.object(sf, "run_ocr") as run_ocr:
